...
import { predict, saveScan } from "../services/predict";  // ✅ uses central config IP
...
export default function HomeScreen({ navigation }) {
  ...
  const pickImage = async () => {
    ...
    // Navigate immediately with undefined result → shows loading spinner
    navigation.navigate("Scan", {
      screen: "Result",
      params: { image: imageUri, result: undefined },
    });
    ...
    try {
      ...
      // Update Result screen with real data
      navigation.navigate("Scan", {
        screen: "Result",
        params: { image: imageUri, result: resultData },
      });
    } catch (error) {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.topBar}>
        <View style={toggleStyles.container}>
          ...
          <TouchableOpacity
            style={[toggleStyles.toggle, language === "en" ? toggleStyles.left : toggleStyles.right]}
            onPress={toggle} activeOpacity={0.8}
          >
            <Text style={toggleStyles.label}>{language === "en" ? "EN" : "हि"}</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      {total > 0 && (
        <View style={progressStyles.card}>
          ...
          <Text style={progressStyles.sub}>{healthy} {strings.of} {total} {strings.cowsHealthy}</Text>
        </View>
      )}
          ...
          renderItem={({ item }) => (
            <View style={cardStyles.wrapper}>
              <View style={cardStyles.card}>
                ...
                <Text style={[cardStyles.disease,
                  item.disease?.toLowerCase() === "healthy" ? cardStyles.healthy : cardStyles.sick
                ]}>
...
const toggleStyles = {
  ...
  background: {
    flexDirection: "row", width: 90, height: 36, borderRadius: 20,
    backgroundColor: "#dcefe4", alignItems: "center",
    justifyContent: "space-between", paddingHorizontal: 10,
  },
  bgText: { fontSize: 12, color: "#1a3c2e", fontWeight: "600" },
  toggle: {
    position: "absolute", width: 45, height: 36, borderRadius: 20,
    backgroundColor: "#1a3c2e", justifyContent: "center",
    alignItems: "center", zIndex: 2, elevation: 5,
  },
...
const statsStyles = {
  ...
  card: {
    ...
    shadowColor: "#a5d6a7", shadowOpacity: 0.15, shadowRadius: 4,
    shadowOffset: { width: 0, height: 2 },
  },
  statNumber: { fontSize: 22, fontWeight: "800", color: "#1b5e20", marginBottom: 4 },
  statLabel: { fontSize: 11, color: "#6a8f6a", fontWeight: "600", textAlign: "center" },
};

const progressStyles = {
  card: {
    backgroundColor: "white", borderRadius: 18, padding: 16, marginBottom: 16,
    borderWidth: 1, borderColor: "#dcedc8", elevation: 3,
    shadowColor: "#a5d6a7", shadowOpacity: 0.2, shadowRadius: 6,
    shadowOffset: { width: 0, height: 3 },
  },
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
  title: { fontSize: 15, fontWeight: "800", color: "#1b5e20" },
  percent: { fontSize: 18, fontWeight: "800", color: "#43a047" },
  barBg: {
    height: 10, backgroundColor: "#f1f8e9", borderRadius: 10,
    overflow: "hidden", marginBottom: 8, borderWidth: 1, borderColor: "#dcedc8",
  },
  barFill: { height: "100%", backgroundColor: "#43a047", borderRadius: 10 },
  sub: { fontSize: 12, color: "#6a8f6a", fontWeight: "500" },
};
...
const sectionStyles = {
  ...
  title: { fontSize: 18, fontWeight: "800", color: "#1b5e20", marginBottom: 5, letterSpacing: 0.3 },
  line: { height: 2, backgroundColor: "#a5d6a7", width: 60, borderRadius: 5 },
  seeAll: { fontSize: 13, color: "#43a047", fontWeight: "700" },
};

const cardStyles = {
  wrapper: { marginRight: 12 },
  card: {
    backgroundColor: "white", borderRadius: 18, padding: 16, width: 150,
    borderWidth: 1, borderColor: "#dcedc8", elevation: 3,
    shadowColor: "#a5d6a7", shadowOpacity: 0.2, shadowRadius: 6,
    shadowOffset: { width: 0, height: 3 },
  },
  iconCircle: {
    backgroundColor: "#f1f8e9", width: 44, height: 44, borderRadius: 12,
    alignItems: "center", justifyContent: "center", marginBottom: 10,
  },
  icon: { fontSize: 24 },
  breed: { fontSize: 14, fontWeight: "800", color: "#1b5e20", marginBottom: 4 },
...
const emptyStyle = {
  text: { textAlign: "center", color: "gray", marginVertical: 16, fontSize: 14 },
};